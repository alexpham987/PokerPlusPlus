...
chat_message chat_session::turn(int turn, std::string/*boost::uuids::uuid*/ id_player/*, std::string name*/)
{
  nlohmann::json to_player;
  chat_message msg;
  std::string json_str;
...
//deal cards
  ...
  if((phase == 1 && game == 1) || info["event"] == "dealt")
  {
    if( turn_index == 0 && info["event"] == "dealt" )
    {
      ...
      room_.deliver( turn( 2, room_.id_players[*(room_.participants_it)] ) );
      return;
    }
  ...
  else if(phase == 1 && game == 0)
  {
    ...
    if( (info["event"] == "check") || (info["event"] == "fold") || (info["event"] == "bet") )
    {
      ...
      if( turn_index == (PLAYERS-1) )
      {
        ...
        room_.deliver( turn( 3, room_.id_players[*(room_.participants_it)] ) );
      }
      else
      {
        turn_index++;
  ...
  else if(phase == 1 && game == 2)
  {
    ...
    if( (info["event"] == "stand") || (info["event"] == "fold") || (info["event"] == "exchange") )
    {
      ...
      if( turn_index == (PLAYERS-1) )
      {
        ...
        room_.deliver( turn( 4, room_.id_players[*(room_.participants_it)] ) );
      }
      else
      {
        turn_index++;
